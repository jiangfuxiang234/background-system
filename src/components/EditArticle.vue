<template>
    <div>
        ...
        <editor ref="toastuiEditor" :initialValue="form.editorText" height="600px" :options="editorOptions" />
...
import "@toast-ui/editor/dist/i18n/zh-cn"; // markdown编辑器语言插件
export default {
    props: ['mode'], // 看是编辑还是添加
    components: {
        editor: Editor,
    ...
    created() {
        ...
        if (this.mode === 'edit') {
            ...
            findOneBlog(this.id).then(({ data }) => {
                this.form = data;
                this.form.select = this.form.category ? this.form.category.id : '';
                this.$refs.toastuiEditor.invoke('setHTML', this.form.htmlContent)
            })
            this.buttonContenner = '编辑文章'
        }
    },
    methods: {
        fecthData() {
