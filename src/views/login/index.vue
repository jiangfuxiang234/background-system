...
export default {
  ...
  data() {
    ...
    return {
      svg: '',
      loginForm: { // 这里犯了一个错误，传给后端api的key名要按后端固定的，不能自己取
        loginId: '',
        loginPwd: '',
        captcha: '',
        checked : true,
      },
      loginRules: { // 验证规则，验证规则对应prop属性
        loginId: [{ 
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          if(this.loginForm.checked){ // 七天免登陆
            this.loginForm.remenber = 7
          }
          this.$store.dispatch('user/login', this.loginForm).then(() => {
            this.$router.push({ path: this.redirect || '/' })
            this.loading = false
          }).catch((res) => { // 验证码错误返回的类型是string，账号密码错误返回的是object
            if(typeof res === 'string'){
              this.$message.error('验证码错误')
            }else{
              this.$message.error('账号密码错误')
            }
            this.getCaptchaFunc(); // 输错点登录后要重新刷新验证码
            this.loading = false;
